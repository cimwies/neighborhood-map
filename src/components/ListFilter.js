    ...
    render = () => {
        ...
        return (
            <aside className="list-wrapper" >
                <button aria-label="Close button of the filter list"  id="close-btn" className="close-list-wrapper-btn" onClick={() => this.closeList()}>
                </button>
                ...
                <div className="list-wrapper-content">
                    <ul tabIndex="0" role="tablist" aria-label="List of beautiful places" className="list-of-places">
                        {locationsGoogle.filter( location => location.marker.visible === true).map((location, index) => (
                            <li tabIndex="0" role="button" key={index} onKeyPress={(event) => this.handlerKeyPress(event, location)} onClick={(e) => this.setMarker(location)}> {location.props.name} </li>
                        ))}
                    </ul>
                </div>
            </aside>
